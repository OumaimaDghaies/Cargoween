...
def geocode_addresses(df):
    if 'Postal Address' not in df.columns:
        return df
    ...
    for idx, row in df.iterrows():
        try:
            query = f"{row.get('Ville', '')}, {row.get('Pays', '')}"
            time.sleep(1)
...
def calculate_tariffs(df, nearest_companies, delivery_distance_km):
    tariffs = []
    for _, company in nearest_companies.iterrows():
        prix_km = float(company['Prix KM'])
        prix_arrivee = float(company['prix arrivée'])
...
def find_nearest_companies_ui(df, airport_coords=None, delivery_address=None):
    ...
    try:
        ...
        print(nearest[['Company Name', 'Postal Address', 'distance_km']].to_string(index=False))
        ...
        if delivery_address:
            ...
            if distance:
                tariffs = calculate_tariffs(df, nearest, distance)
                ...
                # Chargement et application du modèle ML
                try:
                    model = load_model()
                    print("\nChargement du modèle ML réussi")
                    
                    # Application du modèle
                    ml_results = apply_ml_model(model, tariffs.copy(), {
                    ...
                    print(f"\nErreur lors de l'application du modèle ML: {str(e)}")
            else:
...
def main():
    # 1. Traiter le PDF
    document_pdf = "reservation2.pdf"
    address_delivery = "80 Bd Maxime Gorki, 93240 Stains, France"
    
    pdf_data = process_pdf(document_pdf)
    if not pdf_data:
    ...
    # 3. Charger les données nécessaires depuis MongoDB
    try:
        client = MongoClient(MONGO_CONNECTION_STRING)
        db = client[DB_NAME]
        df = pd.DataFrame(list(db[COLLECTION_NAME].find({})))
        
        # 4. Utiliser les données extraites du PDF
