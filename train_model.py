...
def train_and_save_model():
    ...
    try:
        ...
        logging.info(f"Connexion à MongoDB - DB: {DB_NAME}, Collection: {COLLECTION_NAME}")
        
        # Exécution du pipeline
        best_model_name = run_ml_pipeline(
        ...
        if best_model_name:
            logging.info(f"Meilleur modèle sélectionné: {best_model_name}")
            
            # Vérification et sauvegarde du modèle
            if os.path.exists("best_model.pkl"):
                model_size = os.path.getsize("best_model.pkl") / (1024 * 1024)  # Taille en MB
                logging.info(f"Modèle sauvegardé (taille: {model_size:.2f} MB)")
        ...
        logging.error(f"Échec de l'entraînement: {str(e)}", exc_info=True)
        return False
